...
#include <ctime> // for time randomization usage
...
int main (int argc, char* argv[]) // i've finally understood what it means (argc -- number of arguments, argv -- strings of arguments (including -<word> and --<word>))
{
	...
	cout << "Server" << endl;
	string tmp_str_1 = "FileForMap";
	string tmp_str_2 = "map";
	...
	HANDLE MapFile = CreateFileMapping(
		...
		0, // MaximumSizeHigh
		8,//sizeof(int), // MaximumSizeLow
		tmp_str_2.c_str() // MapName
	);
	cout << "HERE 2";
	int *fmap = (int *)MapViewOfFile(
		MapFile, // MapHandle
		FILE_MAP_WRITE, // Access
		0, // OffsetHigh
		0, // OffsetLow
		0//sizeof(int) // CountMapByte
	);
	cout << fmap << " <- result";
	const int size = 500;
	int matrix[size];
	...
	cout << "Client" << endl;
	HANDLE MapFile2 = OpenFileMapping(
		FILE_MAP_WRITE, // Access
		false, // InheritHandle
		tmp_str_2.c_str() // MapName
	);
	cout << "HERE 4";
	int *fmap2 = (int *)MapViewOfFile(
		MapFile2, // MapHandle
		FILE_MAP_WRITE, // Access
		0, // OffsetHigh
		0, // OffsetLow
	...
	cout << fmap2 << " <- result " << GetLastError() << "\n";
	//DWORD dwFileSize = GetFileSize(MapFile2, NULL);
	for (int i = 0; i < 500/*dwFileSize*/; i++)
	{
		cout << "DID IT";
		cout << fmap2[i] << " ";
	}
